define(function () {
	var ActiveData = Class.extend({
		...
		unpack: function (reduce) {
			...
			var idx = 1;
			var rows = []
			if (reduce.rows) for (var rowKey = 0; rowKey < reduce.rows.length; rowKey++) {
				var row = {idx: idx};
		...
		*/
		...
		getCopy: function (expr) {
			...
			var copyData = new ActiveData(JSON.parse(JSON.stringify({columns: this.columns, rows: []})));
